...
class ModelToolExportImport extends Model {
	...
	protected function validateProductIdColumns( &$reader ) {
		...
		$worksheets = array( 'AdditionalImages', 'Specials', 'Discounts', 'Rewards', 'ProductOptions', 'ProductOptionValues', 'ProductAttributes', 'ProductFilters' );
	...
	protected function validateStoreIds( &$reader ) {
		...
		foreach ($worksheets as $worksheet) {
			...
			$k = $data->getHighestRow();
			for ($i=1; $i<$k; $i+=1) {
				$store_id = $this->getCell($data,$i,3);
...
//		}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_3_7&language_code=$language_code");
